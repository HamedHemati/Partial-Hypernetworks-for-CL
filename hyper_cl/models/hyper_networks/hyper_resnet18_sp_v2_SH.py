...
class ResNet18_p2(nn.Module):
    def __init__(self, block=BasicBlock, num_blocks=[2, 2, 2, 2],
                 num_classes=10, nf=20, in_planes=20) -> None:
        super(ResNet18_p2, self).__init__()
...
class HyperResNet18SPv2SH(MultiTaskModule, nn.Module):
    def __init__(self, num_tasks=10, num_classes=10, embd_dim=32,
        ...
        # Main network
        self.feat_extractor_sf = ResNet18_p1()

        self.feat_extractor_sl = ResNet18_p2(in_planes=self.feat_extractor_sf.in_planes,
    ...
    def forward(self, x, task_ids):
        feat = self.feat_extractor_sf(x)

        feat = feat.unsqueeze(1)
        gen_params = self.weight_generator(task_ids)
        out = self.mn_func_vmap(gen_params, feat)
        out = out.squeeze(1)
...
if __name__ == "__main__":
    model = HyperResNet18SPv2SH(num_tasks=10,
                                num_classes=10,
                                embd_dim=32,
                                hidden_size_1=50,
                                hidden_size_2=32,
